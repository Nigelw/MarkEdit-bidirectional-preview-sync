...
- Mirrored a settled preview selection from its rendered range rather than the DOM anchor/focus. Double-clicking a word and dragging back to exactly the word's start left the whole word highlighted in the preview while WebKit reported a collapsed anchor/focus (the raw drag endpoints); the mirror read that as "no selection" and cleared the editor. The selection extent is now taken from `Selection.getRangeAt(0)` (what is actually rendered as selected), and anchor/focus are used only to recover the drag direction, so the word still mirrors.
