...
| ID | Name | Description | Status |
| --- | --- | --- | --- |
| SM-029 | Table plain cell text | Select rendered `URL is hidden in preview` inside a table cell. | Pass: `markdownPlainChars` now strips the table's `|` cell delimiters and cell padding and drops the entire header separator row (`| --- | --- | --- |`, which renders as a rule, not text). The whole `<table>` is a single mapped block (only `<table>` carries `data-line-from`/`data-line-to`; `tr`/`td` do not), so its rendered text is the row-major concatenation of every cell — which now aligns character-for-character with the stripped source, mirroring the exact cell content. Verified via a deterministic harness against the real rendered table DOM. |
| SM-030 | Table bold cell text | Select rendered `bold cell text` in the Emphasis row. | Pass: composes with the existing bold-marker stripping — the pipes, padding, and both `**` wrappers are all excluded, so the mirror is exactly `bold cell text`. Verified live (line 29 highlights `bold cell text` only) and via harness. |
| SM-031 | Table inline code cell | Select rendered `cell_code()`. | Pass: composes with the existing inline-code stripping — the backtick delimiters and the surrounding pipes/padding are excluded and the contents (including `()`) stay literal, so the mirror is exactly `cell_code()`. Verified via harness. |
| SM-032 | Table link cell | Select rendered `cell link`. | Pass: composes with the existing link stripping — the `[`/`]` brackets, the `(destination)` URL, and the surrounding pipes/padding are all excluded, so the mirror is exactly `cell link`. Verified via harness. |
| SM-033 | Across table cells | Select from one rendered table cell into the next. | Expected limitation: both endpoints now resolve to the exact cell characters, but a single contiguous editor range that spans from one cell into the next necessarily contains the interior `|` delimiter (and any cell markers) that sit between the two cells in source — e.g. selecting from `Emphasis` into `bold cell text` mirrors `Emphasis | **bold cell text`. This is the same contiguous-range constraint as SM-016/SM-021/SM-028; selecting within a single cell (SM-029–SM-032) is clean. |
...
| SM-065 | Sentence text after a footnote reference | Select rendered `Try selecting the reference marker` (the sentence text that follows the inline `[^selection-footnote]` reference in the same paragraph). | Pass: the reference marker renders as `[1]` (real text, 3 chars) but its source label `[^selection-footnote]` is 21 chars, so the plain stream used to emit the ~19-char label as content and desync everything after it. The block's rendered marker width is now measured from its DOM (`.footnote-ref` `textContent.length`) and exactly that many placeholder characters are reserved for the `[^name]` span, so text after the marker mirrors to its exact source. The marker itself is unchanged (SM-047 limitation). |
