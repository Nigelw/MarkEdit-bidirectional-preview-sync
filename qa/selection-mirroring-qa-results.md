...
| SM-062 | Backward drag out of a bold span | Double-click `bold` inside `**bold text**`, then drag backward past the start of the word into the preceding space. | Pass (previously failed): the mirror no longer collapses to just the space (or disappears entirely) as the drag crosses the span's opening `**`. When the upper selection boundary sits at the span's content start and the selection is anchored inside the span, the mirror now extends to cover the span (`**bold text`, opening markers included per the contiguous-range limitation) instead of the degenerate single-space range. Transient mid-drag collapses no longer clear the editor selection. Selections that merely touch the boundary from outside (SM-008) are unchanged. |
