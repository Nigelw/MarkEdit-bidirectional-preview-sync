...
| SM-063 | Backward drag to exactly the word start | Double-click `bold` inside `**bold text**`, then drag backward to exactly the start of the word (not into the preceding space) and release, leaving `bold` still highlighted in the preview. | Pass (previously failed): at this boundary WebKit reports a collapsed anchor/focus (the raw drag endpoints coincide with the double-click origin) even though the rendered range still spans the whole word, so the mirror read it as an empty selection and cleared the editor. The selection extent is now taken from `Selection.getRangeAt(0)` rather than anchor/focus, so the still-highlighted `bold` mirrors correctly. Root cause verified from the rendered highlight (the range is non-collapsed while `selection.isCollapsed` is true); the exact word-granularity drag could not be synthesized through automated input, so the fix was verified via the range-based mapping and the unchanged double-click case. |
